...
function ShortCuts() {
  return (
    <div className="flex gap-4 flex-col bg-white rounded-lg p-4 dark:bg-gray-600">
      <Title>Shourtcuts</Title>
      {
        shortcutLink.map((list, index) => (
          <div key={index} className='flex justify-between items-center cursor-pointer rounded-sm'>
            <div className='flex gap-4 items-center'>
              <span className='bg-blue-100 p-2 rounded-full w-8 h-8 flex items-center dark:bg-gray-800 dark:text-gray-300'>
                <list.icon/>
              </span>
              <h3 className="font-semibold dark:text-gray-300">{list.title}</h3>
            </div>
